...
1. Prefer the smallest correct change set. Do not refactor unrelated code.
2. Read only the phase-scoped docs needed for the active task; avoid spec-wide reads.
3. Use `rg`/`rg --files` for discovery and keep edits scoped to requested work.
4. Preserve existing uncommitted changes unless explicitly told to modify them.
5. Verify with targeted tests first, then phase gate (`make test-phaseN`) before broader suites.
6. Report exactly what changed, what tests were run, and which gate criteria were met.
