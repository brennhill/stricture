...
3. Generates a skeleton `strict-lineage.yml` with:
...
```yaml
# strict-lineage.yml (generated by strict init service)
...
|------|---------|-------------|
| `--output` | `strict-lineage.yml` | Output file path |
...
4. Generates `strict-source` annotations (inline or sidecar) with
...
2. Existing `strict-source` annotations and sidecar files.
