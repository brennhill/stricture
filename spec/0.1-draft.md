...
1. Flow catalog is defined centrally (for example `'strict:flows'[]`) with:
   - `id`
   - `name`
...
Reference handles used in documentation/tooling UX SHOULD use `strict:*`
namespace forms (for example `strict:source`, `strict:systems[]`,
`strict:source-edge`, `strict:lineage-override`). Parser syntax in source code
remains token-based (for example `stricture-source` comments and
`stricture-lineage-override` comments).
...
1. `'strict:flows'` definitions at registry scope
