...
6. Policy distribution and offline-consumable governance.
...
### 3.1.2 Business Flow Tiers (Service-Level)

Implementations SHOULD support named business flows with numeric criticality
levels to represent business risk tiers (for example checkout tier 1, support
tier 3).

Normative model:

1. Flow catalog is defined centrally (for example `flows[]` in registry files) with:
   - `id`
   - `name`
   - `level` (integer; organization-defined scale)
2. Service membership is declared on systems (for example `systems[].flows`).
3. Findings derive affected flows from impacted services and lineage paths.
4. Per-API flow-tier tagging SHOULD NOT be the baseline model, because service
   topology + lineage path evaluation provides safer impact classification.

### 3.1.3 Deployment Ledger (Flow-Aware)

Implementations SHOULD support an append-only deployment ledger so policy and
flow views can show when each service in a flow was last deployed.

Normative model:

1. Deployment records MUST include:
   - `service_id`
   - `environment`
   - `deployed_at` (timestamp)
   - `version` (string)
2. Deployment records SHOULD include:
   - `commit` (string)
   - `flow_ids` (array of flow IDs the service participates in)
3. Deployment records MAY include rollout detail:
   - `region` (string or list)
   - `rollout.strategy` (canary, blue_green, ring, custom)
   - `rollout.percent` (0–100)
   - `rollout.status` (in_progress, completed, rolled_back)
   - `rollout.started_at`, `rollout.updated_at`, `rollout.completed_at`

Ingress:

1. Deployments MAY be pushed manually or by CI/CD.
2. Webhook ingestion is OPTIONAL.

Flow-level view:

1. Implementations SHOULD compute flow-level “last deployed” based on the
   most recent deployment for each service in that flow.
...
Namespace convention: source code comments use the `stricture-` prefix (for
example `stricture-source`, `stricture-lineage-override`). YAML configuration
keys use the `stricture_` prefix (for example `stricture_policy_url`,
`stricture_flows`).
...
1. `from` (the upstream service name)

All other canonical properties MAY be omitted in source comments and filled by
deterministic defaults during normalization. Optional qualifiers include `scope`,
`provider`, `transform`, `merge`, `classification`, `break_policy`, `contract`,
and `note`.
...
5. `transform`: `passthrough`
6. `merge`: `single_source` for one source, `priority` for multiple
...
A source entry is a structured object with named fields:

- `kind`: source kind (required)
- `target`: upstream service or endpoint identity (required)
- `path`: specific field path within the source (optional)
- `scope`: relationship scope (required)
- `contract`: contract reference URI (required)
- `provider`: external provider identifier (conditional)
- `as_of`: freshness timestamp (conditional)
...
1. `contract` MUST be present.
2. `provider` MUST be present for `external` sources.
3. `as_of` MUST be present for `external` sources.
4. `provider` MUST NOT be set for non-external sources.
...
When service registries include flow metadata, implementations SHOULD support:

1. `flows` definitions at registry scope
2. `systems[].flows` membership references

System IDs in `systems`, `fields[].source_system`, and `edges` MUST use the same
canonical identifier space, including optional hierarchical IDs from §3.1.1.
...
Default finding model:

1. Implementations SHOULD default to `require_downstream_impact=true`.
2. `self_only` drift SHOULD be tracked/published but SHOULD NOT warn/block by
   default.
3. `unknown` impact SHOULD default to low-severity finding unless policy
   overrides.

Optional flow-criticality policy behavior:

1. Policy MAY compute effective flow level from affected services.
2. Policy MAY override severity by effective flow level.
3. Policy MAY hard-block listed critical flow IDs with explicit reason.
...
fields:
  - field_id: response_payment_status
    ...
    sources:
      - kind: api
        target: payments.GetPayment
        path: response.status
        scope: cross_repo
        contract: git+https://example.com/payments/openapi.yaml@abc
